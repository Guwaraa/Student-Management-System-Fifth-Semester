...
@section('content')
    ...
    <body>
        <div style="background-color:slategray">
            <a href="{{ route('examresult.terminalexam') }}" class="sem" style="margin-left:50px">Terminal
                Exam</a>
            <a href="{{ route('examresult.boardexam') }}" class="sem"
                style="float:right;margin-right:50px;">Board Exam</a>
        </div>
        @if (auth()->user()->status != 'user')
            <div class="header">
                ...
                <form action="{{ route('examresult.boarddisplay') }}" method="POST">
                    @csrf
        ...
        <div class="">
            ...
            <div class="floating-form" id="formpage">
                ...
                <form action="{{ route('examresult.boardstore') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                ...
                                <div style="display: flex; justify-content: space-evenly; width:100%; height:10%; padding: 5px;color:black"
                                    ...
                                    <div style=" width: 75%; display: flex;font-size: 16px;">
                                        <div style="width: 70%;padding: 4px;font-size: 20px;">
                                            <a href="{{ route('examresult.boardedit', [$data->id]) }} " target="_blank"
                                                class="show">
                                                <div>
                                                    {{ $data->title }}
                                                </div>
                                    ...
                                    <div style=" width: 15%;  display: flex;justify-content:flex-end; text-align: right;">
                                        <div style="width: 40%; font-size: 20px;padding-top: 10px; " class="option"
                                            id="option">
                                            <form action="{{ route('examresult.boarddownload') }}" method="POST">
                                                @csrf
                                        ...
                                        @if (auth()->user()->status != 'user')
                                            <div style="width: 40%; font-size: 20px;padding-top: 10px;"
                                                ...
                                                <form action="{{ route('examresult.boarddeletedata') }}" method="POST">
                                                    @csrf
